...
export const SignTxInSiteButton = ({
  ...
  return <>
    <button className='landing-button m-2' style={{ width: 200 }}
      disabled={!chain.address || !chain.loggedIn || (signInMethodTab == 'web3' && web3SignInType == 'siwbb')}
      onClick={async () => {
        if (txDetails.sender.accountNumber < 0) {
          alert('Account number is -1. Accounts need to be registered on the blockchain before signing transactions. To register, you can send them any amount of $BADGE (the account will also need it to pay for gas fees).');
          return;
        }
        ...
        fetchAccountsWithOptions([{ address: chain.address, fetchBalance: true, fetchSequence: true }], true);
      }}>
      Sign Transaction
    </button>
  </>
...
export const BroadcastTxPopupButton = ({
  ...
  return <>
    <button className='landing-button m-2' style={{ width: 240 }}
